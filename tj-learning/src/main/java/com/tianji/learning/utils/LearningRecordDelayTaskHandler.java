...
public class LearningRecordDelayTaskHandler {
    ...
    static ThreadPoolExecutor poolExecutor = new ThreadPoolExecutor(8,
            10,
            60,TimeUnit.SECONDS,
            new LinkedBlockingQueue<>(10));
    // 项目启动后，当前类实例化后，属性注入值之后，方法就会运行，一般做初始化工作
    @PostConstruct
    public void init(){
        log.info("LearningRecordDelayTaskHandler init执行");
        // 开启新线程执行handleDelayTask
        CompletableFuture.runAsync(this::handleDelayTask);
    }
    @PreDestroy // 当前类实例销毁之前 该方法执行
    public void destroy(){
